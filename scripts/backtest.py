...
def run_backtest():
    ...
    if 'game_date' in df.columns:
        ...
        if len(seasons) >= 3:
            ...
            for i, test_season in enumerate(seasons[2:], start=2):
                train_seasons = seasons[:i]   # 用之前所有赛季训练
    ...
    if 'closing_odds' in df.columns and df['closing_odds'].notna().sum() > 0:
        ...
        print(f"\n💹 收盘盘口价值 (CLV)")
        print(f"  平均 CLV: {avg_clv:+.4f} (正值为模型优于收盘线)")
        print(f"  击败收盘盘口比例: {beat_rate:.1%}")
        if avg_clv > 0.02:
            print("  ✅ 模型长期可能拥有真实 edge")
        elif avg_clv < -0.02:
            print("  ⚠️ 模型定价能力不足，需优化")
    ...
    if os.path.exists(FEATURE_IMPORTANCE_LOG):
        ...
        if len(fi_df) >= 2:
            ...
            prev = fi_df.iloc[-2]
            diffs = {}
            for col in fi_df.columns[:-1]:   # 最后一列是时间戳
            ...
            if sorted_diffs and sorted_diffs[0][1] > 0.05:
                print("  ⚠️ 有特征重要性大幅漂移，建议检查数据分布")

    # ========== 6. Bankroll Monte Carlo ==========
    print("\n🎲 资金曲线蒙特卡洛模拟 (1000次 Bootstrap)")
    if 'profit_0.25' in df.columns:
        monte_carlo_bankroll(df['profit_0.25'].values)
    else:
        # 如果没有预计算利润，则用简单凯利计算利润序列
        profit_series = []
