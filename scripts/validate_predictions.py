...
def generate_predictions(elo_system=None):
    ...
    for _, game in schedule_df.iterrows():
        ...
        if nrfi_prob is None:
            if (pitcher_data and 
                pitcher_data.get('home_first_era') is not None and 
                pitcher_data.get('away_first_era') is not None and
                home_top3_woba != 0.320 and away_top3_woba != 0.320):
                home_first_era = float(pitcher_data['home_first_era'])
                away_first_era = float(pitcher_data['away_first_era'])
