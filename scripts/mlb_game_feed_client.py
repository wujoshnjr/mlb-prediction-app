...
def _extract_first_pitcher(
    ...
    confirmed = _game_has_started_or_finished(game_status, detailed_state)
...
def _parse_feed_data(game_id: Union[int, str], payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    home_starting_pitcher_id, home_starting_pitcher_name, home_starting_pitcher_confirmed = _extract_first_pitcher(
        _safe_get(home_box, "pitchers", default=[]),
        home_players_map,
        game_status,
        detailed_state,
    )
    away_starting_pitcher_id, away_starting_pitcher_name, away_starting_pitcher_confirmed = _extract_first_pitcher(
    ...
    home_top3_player_ids_str = ",".join(str(player_id) for player_id in home_batting_order_ids[:3])
    away_top3_player_ids_str = ",".join(str(player_id) for player_id in away_batting_order_ids[:3])
