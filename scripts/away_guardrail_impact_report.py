...
def _base_report(
    ...
    return {
        ...
        "interpretation": {
            "guardrail_note": (
                "Away guardrail impact is measured on current prediction candidates, "
            ...
            "recommended_use": (
                "Use this report to monitor how many away candidates are downgraded "
                "to tracking-only before evaluating settled outcomes later."
            ),
        },
        "warnings": [],
        "errors": [],
...
def compute_report(
    ...
    report["summary"]["prediction_count"] = len(predictions)
    ...
    for prediction in predictions:
        ...
        guardrail_status = _status_text(
            prediction.get("away_guardrail_status", "missing")
        ).lower()
        status_counter[guardrail_status if guardrail_status else "missing"] += 1

        reasons = prediction.get("away_guardrail_reasons")
    ...
    away_guardrail_applied_count = len(guardrail_applied)

    report["summary"].update(
        {
            "away_candidate_count": away_candidate_count,
            "away_guardrail_applied_count": away_guardrail_applied_count,
            "away_guardrail_applied_rate": (
                away_guardrail_applied_count / away_candidate_count
        ...
        downgraded_examples.append(
            {
                ...
                "away_guardrail_reasons": _as_list(
    ...
    if diagnostic_report is not None:
        ...
        away_picks = (
            official_accuracy.get("away_picks", {})
            if isinstance(official_accuracy, dict)
            else {}
        )

        report["diagnostic_snapshot"] = {
            "away_pick_accuracy": (
                away_picks.get("accuracy")
                if isinstance(away_picks, dict)
                else None
            ),
            "away_pick_sample_count": (
                away_picks.get("sample_count")
                if isinstance(away_picks, dict)
                else None
            ),
            "away_pick_rate": (
            ...
            "recommended_guardrails": _as_list(
                diagnostic_report.get("recommended_guardrails")
            ),
        }

    if away_guardrail_applied_count > 0:
        recommendations.append(
