# prediction.py
import os, sys, json, traceback, csv
...
try:
    ...
    if init_shap_explainer(): shap_explainer = True
except: pass
...
if config.FEATURE_USE_PITCH_MATCHUP and get_matchup_lookup is not None:
    try: matchup_lookup = get_matchup_lookup()
    except Exception as e: print(f"Matchup lookup 失败: {e}")
...
if config.RATINGS_ENGINE == 'glicko2' and load_glicko2_league is not None:
    ...
    except Exception as e: print(f"Glicko2 加载失败: {e}")

LAST_GAME_FILE = "data/team_last_game.json"
last_game_dict = json.load(open(LAST_GAME_FILE)) if os.path.exists(LAST_GAME_FILE) else {}

TEAM_ID_MAP = {
    "Braves":144,"Orioles":110,"Red Sox":111,"Cubs":112,"White Sox":145,"Reds":113,
    "Guardians":114,"Rockies":115,"Tigers":116,"Astros":117,"Royals":118,"Angels":108,
    "Dodgers":119,"Marlins":146,"Brewers":158,"Twins":142,"Mets":121,"Yankees":147,
    "Athletics":133,"Phillies":143,"Pirates":134,"Padres":135,"Giants":137,"Mariners":136,
    "Cardinals":138,"Rays":139,"Rangers":140,"Blue Jays":141,"Nationals":120,"D-backs":109
}
TEAM_TIMEZONES = {
    "Braves":"Eastern","Orioles":"Eastern","Red Sox":"Eastern","Cubs":"Central","White Sox":"Central",
    "Reds":"Eastern","Guardians":"Eastern","Rockies":"Mountain","Tigers":"Eastern","Astros":"Central",
    "Royals":"Central","Angels":"Pacific","Dodgers":"Pacific","Marlins":"Eastern","Brewers":"Central",
    "Twins":"Central","Mets":"Eastern","Yankees":"Eastern","Athletics":"Pacific","Phillies":"Eastern",
    "Pirates":"Eastern","Padres":"Pacific","Giants":"Pacific","Mariners":"Pacific","Cardinals":"Central",
    "Rays":"Eastern","Rangers":"Central","Blue Jays":"Eastern","Nationals":"Eastern","D-backs":"Mountain"
}

FEATURE_DIRECTION = {
    'elo_diff':1,'sp_era_diff':-1,'sp_fip_diff':-1,'bullpen_ip_diff':-1,'rest_diff':1,
    'dynamic_park_factor':1,'platoon_ops_diff':1,'statcast_launch_speed_diff':1,
    'statcast_barrel_diff':1,'statcast_hard_hit_diff':1,'statcast_woba_diff':1,
    'timezone_diff':-1,'is_day_game':0,'back2back_diff':-1,'catcher_era_diff':-1,
    'cs_diff':1,'wind_effect':1,'temp_effect':1,'precip_effect':-1,'injury_diff':-1,
    'dynamic_pythag_diff':1,'log5_prob':1,'lag30_winrate_diff':1,'lag30_runs_diff':1,
    'pitch_movement_diff':1,'k_pct_diff':-1,'bb_pct_diff':1,'avg_bat_speed_diff':1,
    'pitcher_rating_diff':1,'odds_change':1,'zone_size':1,'k_rate':1,
    'bullpen_availability_diff':1,'elo_momentum_7d':1,'elo_momentum_30d':1,
    'barrel_pa_diff':1,'hardhit_pa_diff':1,'swing_miss_diff':1,'csw_diff':1,
    'barrel_bb_pct_diff':-1,'sprint_speed_diff':1,'pitch_type_matchup_score':1,
    'top3_woba_diff':1,'winrate_diff':1,'sp_stuff_plus_diff':1,'sp_csw_diff':1,
    'bt_strength_diff':1,'odds_momentum':1,'glicko_rd_sum':1,
...
def generate_predictions(elo_system=None):
    ...
    if not teams_df.empty:
        ...
        if total_games > 0: league_avg_runs = total_runs / total_games
    ...
    if not pitchers_df.empty:
        for _, row in pitchers_df.iterrows(): pitcher_dict[row['game_id']] = row

    bullpen_df = pd.DataFrame(data.get('bullpen', []))
    bullpen_dict = {}
    if not bullpen_df.empty:
        for _, row in bullpen_df.iterrows(): bullpen_dict[row['team_id']] = row

    bullpen_availability_dict = {}
    if calculate_bullpen_availability: bullpen_availability_dict = calculate_bullpen_availability(bullpen_df)

    platoon_df = pd.DataFrame(data.get('platoon', []))
    platoon_dict = {}
    if not platoon_df.empty:
        for _, row in platoon_df.iterrows():
            team, split = row['team_name'], row['split']
    ...
    if not savant_df.empty and 'launch_speed' in savant_df.columns:
        if 'batter_team' in savant_df.columns:
            ...
            for _, row in grouped.iterrows(): statcast_team_stats[row['team_name']] = row.to_dict()
    ...
    if not savant_df.empty and 'pfx_x' in savant_df.columns:
        ...
        if not pitch_df.empty:
            ...
            for _, row in team_pitch.iterrows(): pitch_movement_dict[row['team_name']] = row.to_dict()
    ...
    if not savant_df.empty and 'bat_speed' in savant_df.columns:
        ...
        if not bat_speed_df.empty:
            ...
            for _, row in team_bat.iterrows(): bat_speed_dict[row['team_name']] = row['bat_speed']
    ...
    if not savant_df.empty and 'sprint_speed' in savant_df.columns:
        ...
        if not sprint_df.empty:
            ...
            for _, row in team_sprint.iterrows(): sprint_speed_dict[row['team_name']] = row['sprint_speed']

    pitcher_rating_dict = {}
    if calculate_pitcher_ratings and not savant_df.empty: pitcher_rating_dict = calculate_pitcher_ratings(savant_df)
    ...
    if not savant_df.empty and 'whiff' in savant_df.columns:
        ...
        for _, row in whiff_rate.iterrows(): swing_miss_dict[row['team_name']] = row['whiff_rate']
        if 'csw' in savant_df.columns:
            ...
            for _, row in csw_rate.iterrows(): csw_dict[row['team_name']] = row['csw_rate']
        ...
        for _, row in barrel_rate.iterrows(): barrel_against_dict[row['team_name']] = row['barrel_rate_against']
    ...
    if not injuries_df.empty:
        ...
        for _, row in team_injury.iterrows(): injury_index[row['team_name']] = row['severity_score']

    umpire_df = pd.DataFrame(data.get('umpires', []))
    umpire_dict = {}
    if not umpire_df.empty:
        for _, row in umpire_df.iterrows(): umpire_dict[row['game_id']] = row.to_dict()
    ...
    if os.path.exists(hist_dir):
        ...
        if hist_files:
            try: historical_df = pd.concat([pd.read_parquet(f) for f in hist_files], ignore_index=True)
            except: pass
    ...
    for _, game in schedule_df.iterrows():
        ...
        if get_elo_momentum:
            try:
                home_mom_7 = get_elo_momentum(home,7); away_mom_7 = get_elo_momentum(away,7)
                elo_momentum_7d = home_mom_7 - away_mom_7
                home_mom_30 = get_elo_momentum(home,30); away_mom_30 = get_elo_momentum(away,30)
        ...
        if pitcher_data is not None:
            home_era = float(pitcher_data.get('home_era',4.5) or 4.5)
            away_era = float(pitcher_data.get('away_era',4.5) or 4.5)
            sp_era_diff = home_era - away_era
            home_fip = float(pitcher_data.get('home_fip',4.0) or 4.0)
            away_fip = float(pitcher_data.get('away_fip',4.0) or 4.0)
        ...
        if not platoon_df.empty:
            ...
            if home_platoon and away_platoon: platoon_ops_diff = home_platoon['ops'] - away_platoon['ops']
        ...
        if bat_speed_dict:
            ...
            if home_bs is not None and away_bs is not None: avg_bat_speed_diff = home_bs - away_bs

        sprint_speed_diff = 0.0
        if sprint_speed_dict:
            home_ss = sprint_speed_dict.get(home); away_ss = sprint_speed_dict.get(away)
            if home_ss is not None and away_ss is not None: sprint_speed_diff = home_ss - away_ss
        ...
        if pitcher_rating_dict:
            ...
            if home_rating is not None and away_rating is not None: pitcher_rating_diff = home_rating - away_rating
        ...
        features = {
            ...
            'sp_era_diff': round(sp_era_diff,3), 'sp_fip_diff': round(sp_fip_diff,3),
            'sp_stuff_plus_diff': round(sp_stuff_plus_diff,3), 'sp_csw_diff': round(sp_csw_diff,3),
            'bullpen_ip_diff': round(bullpen_ip_diff,3), 'rest_diff': rest_diff,
            ...
            'timezone_diff': timezone_diff, 'is_day_game': is_day_game,
            'back2back_diff': back2back_diff,
            'catcher_era_diff': round(catcher_era_diff,3), 'cs_diff': round(cs_diff,3),
            'wind_effect': round(wind_effect,4), 'temp_effect': round(temp_effect_raw,4),
            'precip_effect': round(precip_effect,4), 'injury_diff': round(injury_diff,3),
            'dynamic_pythag_diff': round(dynamic_pythag_diff,3), 'log5_prob': round(log5_home,3),
            'lag30_winrate_diff': round(lag30_winrate_diff,3), 'lag30_runs_diff': round(lag30_runs_diff,3),
            'pitch_movement_diff': round(pitch_movement_diff,3),
            'k_pct_diff': round(k_pct_diff,3), 'bb_pct_diff': round(bb_pct_diff,3),
            ...
            'zone_size': round(zone_size,3), 'k_rate': round(k_rate,3),
            'bullpen_availability_diff': round(bullpen_availability_diff,3),
            'elo_momentum_7d': round(elo_momentum_7d,3), 'elo_momentum_30d': round(elo_momentum_30d,3),
            'barrel_pa_diff': round(barrel_pa_diff,3), 'hardhit_pa_diff': round(hardhit_pa_diff,3),
            'swing_miss_diff': round(swing_miss_diff,3), 'csw_diff': round(csw_diff,3),
        ...
        if compute_pitch_usage_features and savant_df is not None and not savant_df.empty:
            if 'pitcher' in savant_df.columns:
                try:
                    pitch_usage_feats = compute_pitch_usage_features(savant_df, home, away,
        ...
        if config.FEATURE_USE_PITCH_MATCHUP and add_matchup_features is not None and matchup_lookup is not None:
            home_sp_id = pitcher_data.get('home_pitcher_id') if pitcher_data else None
            away_sp_id = pitcher_data.get('away_pitcher_id') if pitcher_data else None
            home_top3_ids = game.get('home_top3_ids',[])
            away_top3_ids = game.get('away_top3_ids',[])
            if home_sp_id and away_sp_id:
                features = add_matchup_features(features, home_sp_id, away_sp_id, home_top3_ids, away_top3_ids, matchup_lookup)
        ...
        if config.ODDS_USE_CURVE_FEATURES and extract_odds_curve_features is not None:
            ...
            if game_id:
                ...
                features['home_odds_trend'] = round(trend,3)
                features['home_odds_volatility'] = round(vol,3)
                features['home_odds_reversals'] = int(rev)

        # ---------- 手工集成（修正主场偏置）----------
        # 使用中性 elo（移除主场优势），因为 elo_diff 包含了主场优势
        home_adv = elo_system.home_adv if elo_system else 24
        neutral_elo_diff = features['elo_diff'] - home_adv
        elo_prob_neutral = 1 / (1 + 10 ** (-neutral_elo_diff / 400)) if elo_system or config.RATINGS_ENGINE=='glicko2' else 0.5
        # 手工集成：50% log5, 50% 中性 elo
        manual_pred = log5_home * 0.5 + elo_prob_neutral * 0.5
        sp_adj = -0.07 * sp_era_diff
        manual_pred = min(0.95, max(0.05, manual_pred + sp_adj))

        if elo_system:
            no_odds_pred = log5_home * 0.4 + elo_prob_neutral * 0.6
        else:
        ...
        if historical_count > 10:
            try:
                ...
                if len(hist_df) > 10:
                    ...
                    if len(similar) > 3: pred_std = max(0.02, similar['home_win'].std())
            except: pass
        ...
        if config.NRFI_USE_ML and nrfi_model is not None:
            try:
                nrf_feats = extract_nrf_features({
                    'home_sp_first_inning_era': pitcher_data.get('home_first_era',4.5) if pitcher_data else 4.5,
                    'away_sp_first_inning_era': pitcher_data.get('away_first_era',4.5) if pitcher_data else 4.5,
                    'home_top3_avg_woba': home_top3_woba,
                    'away_top3_avg_woba': away_top3_woba,
                    'umpire_k_rate': k_rate, 'umpire_zone_size': zone_size,
                ...
                nrfi_prob = nrfi_model.predict_proba(pd.DataFrame([nrf_feats]))[0]
            except Exception as e:
                nrfi_prob = None
        ...
        predictions.append({
            "game_id": game.get("game_id"), "game_date": game.get("game_date"),
            ...
            "ci_80_lower": round(ci_80_lower,3), "ci_80_upper": round(ci_80_upper,3),
            "ci_95_lower": round(ci_95_lower,3), "ci_95_upper": round(ci_95_upper,3),
            "simulated_total_mean": total_mean, "simulated_diff_mean": diff_mean,
            ...
            "elo_diff": features['elo_diff'], "sp_era_diff": features['sp_era_diff'],
            "sp_fip_diff": features['sp_fip_diff'], "sp_stuff_plus_diff": features['sp_stuff_plus_diff'],
            "sp_csw_diff": features['sp_csw_diff'], "bullpen_ip_diff": features['bullpen_ip_diff'],
            "rest_diff": features['rest_diff'], "park_factor": static_park,
            ...
            "timezone_diff": features['timezone_diff'], "is_day_game": features['is_day_game'],
            "back2back_diff": features['back2back_diff'],
            "catcher_era_diff": features['catcher_era_diff'], "cs_diff": features['cs_diff'],
            "wind_effect": features['wind_effect'], "temp_effect": features['temp_effect'],
            "precip_effect": features['precip_effect'], "injury_diff": features['injury_diff'],
            "dynamic_pythag_diff": features['dynamic_pythag_diff'], "log5_prob": features['log5_prob'],
            "lag30_winrate_diff": features['lag30_winrate_diff'], "lag30_runs_diff": features['lag30_runs_diff'],
            "pitch_movement_diff": features['pitch_movement_diff'],
            "k_pct_diff": features['k_pct_diff'], "bb_pct_diff": features['bb_pct_diff'],
            ...
            "zone_size": features['zone_size'], "k_rate": features['k_rate'],
            "bullpen_availability_diff": features['bullpen_availability_diff'],
            "elo_momentum_7d": features['elo_momentum_7d'], "elo_momentum_30d": features['elo_momentum_30d'],
            "barrel_pa_diff": features['barrel_pa_diff'], "hardhit_pa_diff": features['hardhit_pa_diff'],
            "swing_miss_diff": features['swing_miss_diff'], "csw_diff": features['csw_diff'],
            ...
            "top3_woba_diff": features['top3_woba_diff'], "winrate_diff": features['winrate_diff'],
