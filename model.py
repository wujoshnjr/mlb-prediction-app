...
class UnifiedSportsModel:
    ...
    def gather_all_data(self, date_str=None):
        if not date_str:
            date_str = datetime.now().strftime('%Y-%m-%d')
        errors = []
        result = {'date':date_str,'errors':errors}
        ...
        mlb = safe_call(fetch_mlb_statsapi, "mlb_statsapi", date_str, errors)
        ...
        result['mlb_statsapi'] = mlb.to_dict(orient='records') if not mlb.empty else []
        ...
        result['pybaseball_statcast'] = []
        result['sportsipy_teams'] = sportsipy.get('teams', pd.DataFrame()).to_dict(orient='records') if isinstance(sportsipy, dict) and not sportsipy.get('teams', pd.DataFrame()).empty else []
